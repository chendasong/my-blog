...
async function handleGenerate() {
  ...
  if (isLoading.value) { abortFlag = true; return }
...
function handleSelect(feature: AIFeature) {
  if (isLoading.value) { abortFlag = true; isLoading.value = false }
  ...
  <div class="ai-page">
    <section class="ai-hero">
      ...
      <div class="ai-hero__inner">
        ...
        <p class="ai-hero__subtitle">集成多种 AI 能力，让创作、分析、开发更高效。</p>
    ...
    <div class="ai-body">
      ...
      <div class="ai-grid">
        <div
          v-for="feature in filteredFeatures" :key="feature.id"
          :class="['ai-tool-card', { 'ai-tool-card--active': selectedFeature?.id === feature.id }]"
          @click="handleSelect(feature)"
        >
          <div class="ai-tool-card__icon">{{ aiIcons[feature.name] || '🤖' }}</div>
      ...
      <Transition name="ai-fade" mode="out-in">
        <div v-if="selectedFeature" :key="selectedFeature.id" class="ai-workspace glass-card">
          ...
          <div class="ai-io">
            <div class="ai-input-area">
              ...
              <div v-if="selectedFeature.id === '5'" class="lang-switcher">
                ...
                <button class="lang-swap-btn" @click="swapLangs" title="交换语言">🔄</button>
...
<style scoped>
.ai-hero { position: relative; padding: 60px 24px 40px; text-align: center; overflow: hidden; }
...
.ai-hero__title { font-size: clamp(2rem,4vw,3rem); font-weight: 700; color: var(--color-text-primary); letter-spacing: -0.03em; margin-bottom: 8px; }
.ai-hero__subtitle { font-size: var(--text-base); color: var(--color-text-muted); margin-bottom: 16px; }
.ai-hero__stats { display: flex; justify-content: center; gap: 12px; flex-wrap: wrap; }
...
.ai-body { max-width: 1200px; margin: 0 auto; padding: 0 24px 80px; }
.ai-cats { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 24px; }
.ai-cat-btn { padding: 7px 16px; border-radius: var(--radius-full); border: 1px solid var(--color-border); background: var(--color-bg-card); font-size: var(--text-sm); color: var(--color-text-secondary); cursor: pointer; transition: all var(--transition-fast); }
...
.ai-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 14px; margin-bottom: 32px; }
.ai-tool-card { display: flex; align-items: flex-start; gap: 14px; padding: 18px 20px; background: var(--color-bg-card); border: 1.5px solid var(--color-border); border-radius: var(--radius-xl); cursor: pointer; transition: all var(--transition-base); position: relative; overflow: hidden; }
.ai-tool-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); border-color: var(--color-border-strong); }
.ai-tool-card--active { border-color: var(--color-primary); background: rgba(91,138,240,0.06); box-shadow: 0 0 0 3px rgba(91,138,240,0.12); }
.ai-tool-card__icon { font-size: 1.8rem; flex-shrink: 0; line-height: 1; }
.ai-tool-card__info { flex: 1; min-width: 0; }
.ai-tool-card__name { font-size: var(--text-sm); font-weight: 600; color: var(--color-text-primary); margin-bottom: 4px; }
.ai-tool-card__desc { font-size: var(--text-xs); color: var(--color-text-muted); line-height: 1.5; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
.ai-tool-card__badge { position: absolute; top: 8px; right: 8px; background: var(--gradient-primary); color: white; font-size: 10px; font-weight: 700; padding: 2px 7px; border-radius: var(--radius-full); }
.ai-workspace { padding: 32px; }
.ai-workspace__header { display: flex; align-items: flex-start; gap: 16px; margin-bottom: 24px; padding-bottom: 20px; border-bottom: 1px solid var(--color-border); }
...
.ai-output__text { padding: 20px; font-size: var(--text-sm); color: var(--color-text-secondary); line-height: 1.8; white-space: pre-wrap; font-family: var(--font-sans); background: rgba(91,138,240,0.02); margin: 0; min-height: 200px; }
...
.ai-output-empty { display: flex; align-items: center; justify-content: center; min-height: 200px; border: 1px dashed var(--color-border); border-radius: var(--radius-lg); color: var(--color-text-muted); font-size: var(--text-sm); }
