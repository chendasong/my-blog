    ...
    <div v-else-if="article" class="container">
      <div class="detail-topbar">
        ...
        <div v-if="authStore.isLoggedIn" class="detail-ops">
          <button class="op-btn" @click="router.push(`/blog/${article.id}/edit`)">✏️ 编辑</button>
        </div>
      </div>
      ...
      <div class="article-footer animate-fade-in-up delay-300">
        <button class="like-btn" @click="handleLike">❤️ {{ article.likes }} 点赞</button>
...
.detail-topbar { display: flex; align-items: center; justify-content: space-between; margin-bottom: 32px; }
...
.article-header__cover { border-radius: var(--radius-xl); overflow: hidden; aspect-ratio: 21/9; margin-bottom: 24px; background: var(--gradient-hero); }
.article-header__cover img { width: 100%; height: 100%; object-fit: cover; }
.article-header__meta { display: flex; flex-wrap: wrap; align-items: center; gap: 12px; margin-bottom: 16px; }
.meta-text { font-size: var(--text-sm); color: var(--color-text-muted); }
.article-header__title { font-size: clamp(1.8rem,4vw,2.8rem); font-weight: 700; color: var(--color-text-primary); line-height: 1.25; letter-spacing: -0.02em; margin-bottom: 16px; }
.article-header__summary { font-size: var(--text-lg); color: var(--color-text-secondary); font-family: var(--font-serif); line-height: 1.7; margin-bottom: 16px; }
.article-header__tags { display: flex; flex-wrap: wrap; gap: 8px; }
.article-body { padding: 40px 48px; margin-bottom: 32px; }
.article-footer { display: flex; align-items: center; gap: 16px; }
.like-btn { display: flex; align-items: center; gap: 6px; padding: 10px 20px; border-radius: var(--radius-full); border: 1px solid var(--color-border); background: var(--color-bg-card); font-size: var(--text-sm); cursor: pointer; transition: all var(--transition-fast); }
.like-btn:hover { border-color: #E8607A; color: #E8607A; transform: scale(1.05); }
